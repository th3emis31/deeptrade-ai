...
Stdlib only. Run it directly for a standalone backtest:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    def cfg_for(inverse: bool) -> Config:
        return Config(initial_capital=args.capital, risk_percent=args.risk,
    ...
    if args.both:
        runs = [(False, "STRATEGY"), (True, "INVERSE BASELINE")]

    for inverse, label in runs:
        cfg = cfg_for(inverse)
        m = metrics(backtest(candles, cfg), cfg)
        print(format_report(m, label))
        print()
        if m["closed_legs"] < 100:
            print("  NOTE: under 100 closed legs — insufficient evidence, whatever the profit factor says.\n")
    return 0
